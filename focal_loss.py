...
def Focal_loss(input, target, gamma=0, alpha=None, size_average=True):
    ...
    # Asegúrate de que los tamaños de input y target sean iguales
    #print(input.size()[-2:], target.size())
    assert input.size()[-2:] == target.size(), "Input and target sizes must match."

    logpt = F.log_softmax(input, dim=1)
    target = target.long()  # Convierte el tensor de destino a tipo de datos int64
    logpt = logpt.gather(1, target)
    logpt = logpt.view(-1)
    pt = logpt.exp()
